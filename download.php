...
function addFileToZip($path,$zip,$localpath){
    $handler=opendir($path); //打开当前文件夹由$path指定。
    while(($filename=readdir($handler))!==false){
        if($filename != "." && $filename != ".."){//文件夹文件名字为'.'和‘..’，不要对他们进行操作
            if(is_dir($path."/".$filename)){// 如果读取的某个对象是文件夹，则递归
                $file_info_arr= pathinfo($path."/".$filename); 
                $currentpath =$file_info_arr['basename'];
                $zip->addEmptyDir($localpath."/".$currentpath); 
                addFileToZip($path."/".$filename, $zip,$localpath."/".$currentpath);
            }else{ //将文件加入zip对象
                $zip->addFile($path."/".$filename,$localpath."/".$filename);  //addFile 的第二个参数(默认使用原始路径名)，ZIP归档中的本地名，它将覆盖文件名
				////$zip->addFile($file,$new_file);   $new_file 将会 覆盖 $file 的文件名，所以我们这里加入相应的路径名即可，即 "路径名" + "/" + "文件名"
             }
        }
    }
    @closedir($path);
...
function create_zip1($files = array(),$destination = '',$overwrite = false) {        
    ...
    if(file_exists($destination) && !$overwrite) { return false; }    
        ...
        if(count($files)) {      
            ...
            foreach($files as $file) { 
                $file_info_arr= pathinfo($file); 
                $localpath =$file_info_arr['basename'];
                if(is_dir($file)){    
                    $zip->addEmptyDir($localpath); 
                    addFileToZip($file,$zip,$localpath);
                }else{
...
file_dir[]="han"; //文件夹和文件名 都行 绝对路径也可以
file_dir[]="hypot"; // 文件夹和文件名 都行 绝对路径也可以
